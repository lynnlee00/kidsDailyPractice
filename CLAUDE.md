...
目前做了 `mult9`(9 的乘法,深入理解單一乘數用,有格子圖+規律心法)當第一版試做,後來(2026-08-21 稍晚)又照同樣格式加了 `mult6`/`mult7`/`mult8`(使用者說這幾個是小孩比較容易搞混的),`unitLabel` 依序是「數學 17」「數學 18」「數學 19」(接在當時已有的 `capacity2`=數學16 後面,沒有往前插進 `mult9`/`multMixed` 中間去打亂既有章節的編號)。這四個乘法章節的資料結構完全一致:`levelPool` 60 題(每題型态跟 `mult9` 一樣是直接背誦/複習/缺格找答案(前後兩種)/拆解法挑戰/生活應用題混合)+ `poolSampleSize:15`,`renderMultiplicationStage()`/`renderRulePanel()`/`renderQuiz()` 這些渲染邏輯完全沒有改,只是資料層面的擴充。

**每個乘數的「拆解法」規律筆記用的分解方式不一樣**(這是特地為每個乘數挑一個好記、算得動的拆法,不是套同一個公式):`mult6` 用「6=5+1,6×n=5×n+n」+「6=3×2 加倍法」;`mult7` 用「7=5+2,7×n=5×n+2×n」+ 跳著數7的口訣(沒有像9那樣乾淨的位數規律,7的乘法本身就是公認比較難背的一個);`mult8` 用「8=10−2,8×n=10×n−2×n」+「連續加倍三次(2×2×2=8)」這個常見教法。`mult6`/`mult8` 也各自寫了個位數字輪流出現的規律(6的乘法個位數是6,2,8,4,0循環;8的乘法個位數是8,6,4,2,0循環),`mult7` 額外提醒「7×7=49、7×8=56、7×9=63」這三個特別容易背混,要多練習。**這批題目是用 Node.js 產生器腳本生成的(跑完就刪了,不在專案裡),每一題的正確答案都用完全獨立於產生邏輯的算式重新算一次來驗證是否一致(不是信任同一段程式碼生出來的答案),也檢查過選項不重複、沒有負數選項——照 CLAUDE.md 既有的「獨立驗證」慣例做的,沒有跳過這一步。**
...
首頁「📕 錯題本」按鈕,右上角會有一個紅色數字徽章顯示目前還沒複習對的題數(`updateMistakeBookBadge()`,每次答對/答錯後都會重新算)。點下去會呼叫 `collectMistakeQuestions()`:把 `PROGRESS.mistakes` 裡每個章節、每個 `questionKey`,拿去**該章節目前的 `levelPool` 裡重新查一次原始題目內容**(不是存一份題目快照)——這樣做的好處是,如果之後修正了某題的文字/答案錯誤,錯題本裡看到的會自動是修正後的版本,不會卡住舊的錯誤內容。查不到的(代表題目後來被刪掉或改到匹配不上)就順便把這筆過期紀錄清掉。收集完一樣組成合成章節、丟進 `renderQuiz()` 引擎;**答對某一題就會自動把它從 `PROGRESS.mistakes` 刪掉**(`handleAnswer` 正確分支本來就有這段邏輯),所以錯題本是「越練越少」、會自己清空,不需要另外寫「標記已複習」的按鈕。如果目前沒有任何錯題,點下去只會跳出一個溫馨的 `alert`,不會進入空白畫面。

**⚠️ 錯題本是照科目分開的(2026-08-21 當天稍晚修正),不是全站混在一起。** 使用者做完第一版(全站混合)之後明確要求「國語歸國語、數學歸數學、英文歸英文」分開。做法:`mistakeCountForSubject(subject)` 只算 `ch.subject === subject` 那些章節的錯題數;`updateMistakeBookBadge()` 用 `currentSubject`(首頁上方目前選的科目)決定要顯示哪個科目的數字跟標籤文字(`#mistakeBookLabel` 會動態顯示「國語錯題本」/「英文錯題本」/「數學錯題本」),`goHome()` 跟切換科目按鈕都會觸發重新整理這個徽章;`collectMistakeQuestions(subject)` 現在需要帶科目參數,只收集該科目的錯題,`startMistakeReview()` 呼叫時帶 `currentSubject`,合成章節的標題/`alert` 文字也都會標明是哪一科的錯題本。**這個分科邏輯完全是在首頁按鈕層級做篩選,`PROGRESS.mistakes` 本身還是全站一份資料(以章節 id 分類),沒有另外拆存檔結構**,之後如果要看某科目的錯題,直接用 `mistakeCountForSubject()`/`collectMistakeQuestions()` 帶對科目參數就好。
...
- `character1` ~ `character12`:康軒二上國字生字第 1~12 課(全部12課都做完了),`category:'character'`,`enabledTabs:['story','game']`。2026-08-22 曾發現 `character5`/`character9`/`character10`/`character11` 這四課的內容跟正確課本對不上、`character12` 原本完全沒有,已經整批重新核對修正,細節見下面「國字生字內容重新核對修正」那節。
- `charDiscrim1` ~ `charDiscrim4`:字音字形練習,依生字課次分組(第1~3課/4~6課/7~9課/10~12課),`category:'charDiscrimination'`,`enabledTabs:['grammar','game']`。第四個單元原本只有10~11課(因為當時第12課還沒做),2026-08-22 已經擴充成10~12課,細節見上面「字音字形練習」那節跟下面「國字生字內容重新核對修正」那節。

### 數學(subject: 'math')
- `mult9`/`mult6`/`mult7`/`mult8`:9/6/7/8 的乘法,`category:'multiplication'`,`enabledTabs:['story','grammar','game']`,深入理解單一乘數用(格子圖+規律心法),每個都是 `levelPool`60題+隨機抽15題。`unitLabel` 分別是數學1、17、18、19(`mult6`/`mult7`/`mult8` 是後來才加的,接在其他章節後面,沒有往前插打亂編號)。
...
### 國字生字內容重新核對修正(2026-08-22)

使用者傳了一份 Google Drive 上的 PDF(「地方爸爸」製作的康軒二上國語生字簿,`康_二上_一般版.pdf`/`康_二上_多描版.pdf`,兩份內容一樣,只是一般版跟描紅練習版的差別),裡面完整列出康軒二上全部12課的生字、注音(全部是注音符號)、部首、筆畫。Claude Code 把這份PDF每一頁都轉成圖片仔細看過,列出全部12課的標題:第一課新學年新希望、第二課一起做早餐、第三課走過小巷、第四課運動會、**第五課坐竹籃船**、第六課小鎮的柿餅節、第七課國王的新衣裳、第八課「聰明」的小熊、**第九課大象有多重?**、**第十課新年快樂**、第十一課遠方來的黑皮(原本這課的編號跟標題就是對的)、**第十二課我愛冬天**。

比對之後發現:**網站原本的 `character5`(標題「水上木偶戲」)、`character9`(標題「等兔子的農夫」)、`character10`(標題「我愛冬天」但標成第十課)、`character11`(標題「庾亮不賣馬」)這四課的生字內容,其實跟康軒二上課本完全對不上**,應該是很早以前用別的資料源做的(不確定是什麼來源)。只有第1~4、6、7、8課的內容是對的。使用者確認後明確指示「以這份(PDF)為主,如果其他課生字有不同的話,就改成這份的規格」。

Claude Code 因此把這四課的 `characters[]` 陣列全部換成跟PDF一致的正確生字(讀音/部首/筆畫都對照PDF圖片核對過,特別小心處理了「圍」「團」的部首是`囗部`不是`口部`這種容易看錯的情況),`words[]`造詞例句是重新編寫的(不是抄課本,課本沒有提供例句),`title`/`unitLabel`也一併修正:
- `character5`:水上木偶戲 → **坐竹籃船**(第五課編號本來就對,只有內容錯)
- `character9`:等兔子的農夫 → **大象有多重?**(第九課編號本來就對,只有內容錯)
- `character10`:我愛冬天(標成第十課)→ **新年快樂**(這才是真正的第十課內容;原本的「我愛冬天」內容經比對後發現其實是接近第十二課主題的內容,但字詞跟PDF的第十二課仍有出入,所以第十二課是另外全新做的,不是把這份舊內容搬過去)
- `character11`:庾亮不賣馬 → **遠方來的黑皮**(第十一課編號本來就對,標題跟內容都錯,已修正)
- 新增 `character12`:**我愛冬天**(這才是PDF裡真正的第十二課,18個生字:絡鼓牠涼灰圍冒居發欣幸聚團咪冰妙鍋愉)

**因為 `charDiscrim4`(字音字形第四單元,原本命名「10~11課」)裡面 15 組配對題目中有 14 組的目標字,其實是引用舊的、錯誤的 `character10`/`character11` 生字內容出的題**,生字內容一換,那些題目就不再對應第10~12課的生字了。Claude Code 已經逐一比對,保留了 4 組原本剛好還是對的（冒、聚、飯、愉，這幾個字剛好也在新的第10/12課生字裡)，其餘 11 組換成從新的第10~12課生字裡挑的同音字組合(例如冬/東、洋/羊/陽、冰/兵/餅、待/代/戴等),單元標題也從「10~11課」改成「10~12課」。

**⚠️ 這次修正過程中曾經發生一次操作失誤**:因為 `character11` 在檔案裡跟 `charDiscrim1` 之間其實隔著全部19個數學章節(`mult9`~`capacity2`~`mult6`/`mult7`/`mult8`),寫替換腳本時誤以為兩者是緊鄰的,結果 `html.indexOf('charDiscrim1')` 找到的是隔著一大段之後的位置,一次操作就把中間所有數學章節都刪掉了。發現後立即從上一次 git commit 的版本裡救回 `mult9`~`capacity2`(16章),`mult6`/`mult7`/`mult8`(這3章因為是這次session裡新加、還沒commit過,git裡也沒有,只能重新用同一套產生器腳本重跑一次補回來),最後有重新測試過全站42個章節都正常才繼續。**之後如果又要用「找下一個章節的id當作替換終點」這種字串搜尋方式改動 CHAPTERS 陣列裡的內容,務必先確認兩個章節在檔案裡是不是真的緊鄰,不能只憑陣列邏辯上的『下一個』就假設兩者中間沒有其他內容——這次的教訓是資料量大的檔案很容易在章節之間夾著其他完全不相關的章節。**
...
- 2026-08-21(加完進度存檔+今日挑戰+錯題本+學習地圖+4個字音字形單元)之後:約 860KB / 8707 行。
- 2026-08-21(加完錯題本分科+`mult6`/`mult7`/`mult8`三個乘法章節)之後:約 909KB / 9062 行。
- 2026-08-22(重新核對修正 `character5`/`9`/`10`/`11`+新增 `character12`+`charDiscrim4` 題庫重建)之後:約 **932KB / 9280 行**。**目前還在安全範圍內(門檻是 1.5MB / 20000 行),但已經超過基準的一半了,之後如果還要繼續擴充題庫或加新章節,記得留意這個數字有沒有接近門檻。**
